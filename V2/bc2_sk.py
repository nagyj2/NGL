# NGL Bytecode 2.0 Grammar Skeleton (Preprocessor)
...
FIRSTPRIMATIVE  = {INT, FLOAT, BOOL, STRING}
...
FIRSTFUNC  = {TYPE, LABEL, LEN, USRFUNC}
...
FIRSTSTMT  = {VAR, SET, GOTO, IF, COMPARE, PRINT, READ, DELETE, TRY, INCLUDE, RETURN}
...
def program():
    ...
    while SC.sym in FIRSTPROGRAM:
        ...
        if SC.sym == IDENT:
            getSym()
            if SC.sym == COLON:
                # if ismod:   name = '_' + SC.val
                name = SC.val
                ST.newJump(name,SC.line); getSym()
...
def arrow():
    ...
    if SC.sym == GOARROW1:
        ST.newJump(RETARROW1,SC.line); getSym()
    elif SC.sym == GOARROW2:
        ST.newJump(RETARROW2,SC.line); getSym()
    elif SC.sym == RETARROW1:
        ST.newJump(GOARROW1,SC.line); getSym()
    elif SC.sym == RETARROW2:
        ST.newJump(GOARROW2,SC.line); getSym()
...
def execute_debug(src, subrun = False):
    global calls, ismod
    # ismod = subrun
