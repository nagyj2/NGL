...
class Value:
    ...
    def __str__(self):
        return '{0}{1}{2}{3}'.format('const ' if self.const == True else '', self.strrep, ' of {0}'.format(str(self.sub)) if self.sub != None else '', ' ({0})'.format(self.data) if self.data != None else '')
...
class Ref(Value):
    ...
    def __init__(self,typ,data=None,const=False):
        ...
        elif typ == 'type':     self.sub = Ref.Type()
    ...
    class Input(Value): # from `in` statement
        strrep = 'input'
    class Ident(Value): # from an unresolved identifier call -> should not exist when interpretation begins
        strrep = 'ident'
    class List(Value): # contents of a list
        strrep = 'list'
    class Special(Value): # an identifier which has special system meaning / varying type
        strrep = 'sys'
    class Function(Value): # a function return
        strrep = 'func'
    class Error(Value): # an identifier only generated by errors
        strrep = 'null'
    class Type(Value): # can hold any value
        strrep = 'type'
...
class Arr(Value):
    strrep = 'array'
    def __init__(self,subtyp=Ref('type'),const=False):
